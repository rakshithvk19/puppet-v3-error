...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        ...
        // 3. Wait for TWAP to update (100 seconds)
        vm.warp(block.timestamp + 100);

        // 4. Calculate the maximum amount we can borrow
        ...
        // 5. Convert remaining ETH to WETH if needed
        uint256 playerWethBalance = weth.balanceOf(player);
        if (playerWethBalance < depositRequired) {
            uint256 ethNeeded = depositRequired - playerWethBalance;
        ...
        // 7. Transfer borrowed tokens to recovery address
